...
def infer_by_trained_gpt(
    ...
    for instance in tqdm.tqdm(dataset):
        ...
        if relation != excluded_relation and instance_id in triggerword and instance_id in newrelation:
            ...
            for new_relation in conduct_new_relation_constrain(new_relations, new_relation_constrain):
                ...
                for output_prompt in output_prompts:
                    ...
                    if generated_trigger_words:
                        ...
                        other_new_relations = []
                        for other_new_relation in conduct_new_relation_constrain(new_relations, 4):
                            if other_new_relation != new_relation:
                                other_new_relations.append(other_new_relation)

                        others_relation_match = False
                        for other_new_relation in other_new_relations:
                            other_relation_words = relationword[other_new_relation]
                            other_relation_match = judge_relation_match(generated_trigger_words, other_relation_words)
                            if other_relation_match:
                                others_relation_match = True
                                break

                        if new_relation_match:
                            if not old_relation_match and not others_relation_match:
